...
<template>
  <form novalidate @submit.prevent="emit('submit', data)" class="signin-form bva-signin-form">
    ...
    <div v-else>
      ...
      <FormInput id="email" placeholder="Email" v-model="data.email" :error="props.errors.email" type="email" class="mt-3" />
      <FormInput placeholder="Username" id="username" v-model="data.username" :error="props.errors.username" class="mt-3" />
      <FormInput placeholder="Password" id="password" v-model="data.password" :error="props.errors.password" type="password" class="mt-3" />
      <FormInput placeholder="Company" id="account_title" v-model="data.account_title" :error="props.errors.account_title" class="mt-3" />
      <div v-if="1 === 2" class="dropdown mt-3 w-100">
        <button :class="[props.errors?.domain_id ? 'invalid-border' : '', 'btn btn-secondary dropdown-toggle w-100']" type="button" data-bs-toggle="dropdown" aria-expanded="false">Account type {{ selectedDomain.title ? ": " + selectedDomain.title : "" }}{{ " " }}</button>
